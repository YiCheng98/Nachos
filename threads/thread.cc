...
{
	if (existingThreadcnt >= 128)
		printf("线程数量已达到上限，无法创建新线程！\n");
	ASSERT(existingThreadcnt < 128);
	...
	existingThreadcnt = existingThreadcnt+1;
#ifdef USER_PROGRAM
    space = NULL;
	userId = userThreadcnt;
	userThreadcnt = userThreadcnt+1;
#endif
}
